...
static int8_t USB_CDC_demo_Init     (void);
static int8_t USB_CDC_demo_DeInit   (void);
static int8_t USB_CDC_demo_Control  (uint8_t cmd, uint8_t* pbuf, uint16_t length);
static int8_t USB_CDC_demo_Receive  (uint8_t* pbuf, uint32_t *Len);

USBD_CDC_ItfTypeDef USBD_CDC_USB_CDC_demo_fops = 
{
  USB_CDC_demo_Init,
  USB_CDC_demo_DeInit,
  USB_CDC_demo_Control,
  USB_CDC_demo_Receive
};
...
/**
  ...
  * @retval Result of the operation: USBD_OK if all operations are OK else USBD_FAIL
  */
static int8_t USB_CDC_demo_Receive(uint8_t* Buf, uint32_t *Len)
{
